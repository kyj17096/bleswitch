...
public class BleWrapper {
    ...
    public void getCharacteristicValue(BluetoothGattCharacteristic ch) {
        		 ...
        		 uuid.equals(BleDefinedUUIDs.Characteristic.FIRMWARE_REVISION_STRING)) // firmware revision string
        {
        	...
        	{
        		strValue =strValue + " "+ Integer.toHexString(0x0ff&rawValue[i]);
        	}
        }
    ...
    private final BluetoothGattCallback mBleCallback = new BluetoothGattCallback() {
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        	...
        	String description = "Device: " + deviceName;
        	
        	// we got response regarding our request to write new value to the characteristic
        	// let see if it failed or not
        	if(status == BluetoothGatt.GATT_SUCCESS) {
        		 mUiCallback.uiSucessfulWriteDescription(mBluetoothGatt, mBluetoothDevice, mBluetoothSelectedService, descriptor, description);
        	}
        	else {
        		 mUiCallback.uiFailedWriteDescription(mBluetoothGatt, mBluetoothDevice, mBluetoothSelectedService, descriptor, description + " STATUS = " + status);
        	}
        };
        @Override
